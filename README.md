...
- **背景を `transform: scaleX(-1)` で左右反転**しています。元画像はラックが左にあり、
  ...
  テキストの左揃えは維持しています。切り出しは `object-position: 10% 50%`（1200px以上は中央）。
- 写真の壁面・床・ラックが見えることを優先し、**覆いは最小限**にしています。

| 層 | 役割 | 値 |
|---|---|---|
| `.hero__media::after` | 画像全体 | `rgba(233,236,234,.30)` 均一（1200px以上は右へ `.30 → .28 → .18 → .04` と抜く） |
| `.hero__copy::before` | 文字の背後だけ | `rgba(233,236,234,.45)`。**テキストの実幅（41rem）+ 左右32px** に限定し、四辺を `mask-image` でぼかす |
| 文字色 | ヒーローの本文・注記 | `--muted` ではなく **`--ink`**（濃い文字ほど覆いを薄くできる） |
| `text-shadow` | 見出し・本文・注記 | `0 1px 3px rgba(255,255,255,.9)` |

局所スクリムは `.hero__copy`（テキストの実幅にフィットするラッパー）に対して敷いており、
`mask-composite: intersect` で左右32px・上下20pxをぼかしているため境界が出ません。
`mask-composite` 非対応環境では矩形の縁が出ますが、コントラストは満たしたままです。

#### コントラストの実測

不透明度は目分量ではなく実測で決めています。ヘッドレスChromeで**文字だけを透明にした状態の
スクリーンショットを撮り、文字の行ボックスに当たるピクセルだけを採取**して、文字色との
コントラスト比を算出しました。`color: transparent` にしても `text-shadow` は描画されるため、
**白いハローを含んだ状態の背景**として測っています。

| 検証幅 | 見出し | 本文 | 注記（13px） |
|---|---|---|---|
| 375×667 / 375×812 | 5.74 | 4.98 | 5.08 |
| 390×844 | 5.52 | 4.98 | 5.08 |
| 412×823 | 5.55 | 4.96 | 5.08 |
| 600px | 4.98 | 4.96 | 5.11 |
| 800px | 5.15 | 4.96 | 7.76 |
| 1024px | 10.7 | 5.02 | 9.23 |
| 1200px | 8.78 | 4.91 | 5.15 |
| 1280px（動画再生中も同値） | 8.94 | 7.41 | 5.19 |
| 1920px | 9.65 | 10.3 | 5.59 |

いずれも**最小値**（もっとも条件の悪い1ピクセル）で 4.5:1 以上です（全体の最小は1200pxの本文 4.91）。

**text-shadow だけでは 4.5:1 に届きませんでした。** ハローはグリフの周囲しか持ち上げないため、
行間や字間の暗いピクセルが残り、4層まで重ねても最小値は 3.49 止まりでした（計測値）。
そのため文字色を `--ink` に変更し、覆いを薄いまま成立させています。
この構成での全体オーバーレイは以前の 0.55 から **0.30** まで下げられました。
