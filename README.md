...
- 可読性は**画像全体を覆う均一オーバーレイではなく、テキストブロック背後の局所スクリムで確保**しています。
  全体オーバーレイを濃くすると写真が死ぬため、次の二層構成にしています。
...
上下はマスク）。パネルを敷いたようには見えず、文字のない上下・右側では写真がそのまま見えます。
文字の背後でも実効不透明度は約 0.92 で、以前の均一 0.95 より写真が見えます。
...
| 検証幅 | 見出し | 本文 | 注記（13px） |
|---|---|---|---|
| 375px | 11.9 | 4.91 | 4.87 |
| 412px | 12.3 | 4.92 | 4.87 |
| 800px | 12.4 | 5.10 | 4.88 |
| 1024px | 12.2 | 4.97 | 4.97 |
