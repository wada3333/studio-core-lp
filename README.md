...
- 可読性は**画像全体を覆う均一オーバーレイではなく、テキストブロック背後の局所スクリムで確保**しています。

| 層 | 役割 | 値 |
|---|---|---|
| `.hero__media::after` | 画像全体。階調を落ち着かせるだけ | `rgba(233,236,234,.55)` 均一（1200px以上は右へ `.55 → .5 → .3 → .06` と抜く） |
| `.hero__inner::before` | 文字の背後だけを持ち上げる局所スクリム | 左は画面外まで `.82` の平坦部、右は文字列の外側（58rem付近）で 0、上下は `mask-image` で消す |

局所スクリムは**四辺すべてで境界が出ない**ように作ってあります（左は画面外、右はグラデーションで 0、
上下はマスク）。パネルを敷いたようには見えず、文字のない上部では写真がそのまま見えます。
...
| 検証幅 | 見出し | 本文 | 注記（13px） |
|---|---|---|---|
| 375×667 / 375×812 | 11.2 | 4.87 | 4.87 |
| 390×844 | 11.2 | 4.87 | 4.89 |
| 412×823 | 11.2 | 4.87 | 4.90 |
| 800px | 11.1 | 4.86 | 5.28 |
| 1024px | 12.6 | 4.86 | 5.36 |
| 1200px | 12.2 | 4.70 | 4.88 |
| 1280px（動画再生中も同値） | 12.2 | 5.12 | 4.88 |
| 1920px | 12.3 | 5.55 | 4.92 |

いずれも**最小値**（もっとも条件の悪い1ピクセル）で 4.5:1 以上です。
**Lighthouse / axe は画像の上に乗る文字のコントラストを判定できない**ため、この計測を別途行っています。
実際、反転前の設定はモバイルで 3.31 まで落ちていましたが、Lighthouse は 100 のままでした。
