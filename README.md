...
`http://localhost:8123/` を開いてください（`127.0.0.1` ではなく `localhost` です。
Cloudflare Turnstile のウィジェットは `localhost` を許可ホストとして登録しているため、
`127.0.0.1` で開くと予約フォームの Turnstile 部分だけ動作しません）。
...
- **Cloudflare Turnstile** — 送信ボタン直前にウィジェットを設置し、トークン未取得のあいだは送信ボタンを `disabled` にする。GAS 側で `siteverify` API に投げて検証し、`hostname` が本番ドメインと一致するかまで確認する。バイパスは設けていない（`localhost` もウィジェットの許可ホストに登録しているため、ローカルでも実チャレンジで検証する）
...
- **日次受付上限** — 1日50件を超えたら受付を停止し、上限到達を検知した最初の1回だけ管理者へメール通知。50件にしているのは MailApp（Gmail）の1日あたり送信上限が100通で、予約者への即時確認メール（最大50通）＋管理者へのまとめ通知1通を送っても上限の半分に収まるようにするため
...
6. **通知トリガーを設定する**
   予約者本人への確認メールは即時送信しますが、管理者への通知は1日1回まとめて送る方式です。
   関数 `installNotifyTrigger` を1度だけ実行すると、`notifyPendingReservations` を
...
9. **確認**
   ...
   予約フォームから実際に1件送信し、次を確認してください。
   - スプレッドシートに行が追加されること
   - `debugDailyCount` で受付件数が加算されていること
   - 入力したメールアドレスへ確認メールが届くこと（`sendConfirmationEmail` の即時送信）
...
- **通知メール** — 予約者本人へは `sendConfirmationEmail` が予約成立の直後に確認メールを
  即時送信します。管理者へは即時送信せず、`notifyPendingReservations` がシートの
  「通知済み」列を見て未通知分をまとめて1通で `MAIL_TO_ADMIN` に送ります
  （`MAIL_TO_ADMIN` が空のままだと管理者向けの送信だけをスキップし、`Logger.log` に記録します）
...
- 02 選ばれる理由の3項目は「番号 → 見出し → 本文 → 画像」で構造を統一し、
  ブロック内の全要素の左端と、本文・画像の幅（65ch）を揃えています。
  画像は全項目 4:3・`object-fit: cover`。余白は 見出し→本文（16px）< 本文→画像（36px）< ブロック間（64〜104px）の順です
...
```css
.hero { min-height: min(78vh, 40rem); min-height: min(78svh, 40rem); }          /* 1200px以上 */
@media (max-width: 74.99em) {
  .hero { min-height: min(88vh, 155vw); min-height: min(88svh, 155vw); }        /* スマホ・タブレット */
}
```

- `svh`（small viewport height）は**アドレスバーの開閉で値が変わらない**ため、`dvh` や `100vh` と違って
  スクロール中にヒーローの高さが動かず、レイアウトシフトが起きません。未対応ブラウザ向けに `vh` を先に宣言しています。
- `155vw` の上限は構図のためです。`object-fit: cover` で見える横幅の割合は
  **「画像の縦横比 ÷ 枠の縦横比」**で決まり、枠を高くするほど横が切れます。
  高さを `1.55 × 幅` に抑えると可視幅が常に元画像の約36%になり、
  ダンベルラック（元画像の 9〜36% に位置）が画面幅に収まります。
  上限を外して `88svh` だけにすると、412×823 の端末では可視幅が31.8%まで狭まりラックが切れます。
- スマホでは `align-items: flex-end` でテキストを下寄せにし、写真の床側と重ねています。

#### 反転とオーバーレイ

- **背景を `transform: scaleX(-1)` で左右反転**しています。元画像はラックが左にあり、
  左揃えの見出しと重なって読みづらかったためです。切り出しは `object-position: 10% 50%`（1200px以上は中央）。
- 右上の窓が白飛びしており、明るい地に濃い文字を乗せる方式では安定しなかったため、
  **暗いオーバーレイ＋白抜き文字**に切り替えています。

| 対象 | 値 |
|---|---|
| `.hero__media::after`（1200px未満） | `linear-gradient(to top, rgba(0,0,0,.80) 0%, rgba(0,0,0,.68) 45%, rgba(0,0,0,.12) 100%)` に `linear-gradient(to right, rgba(0,0,0,.30), transparent)` を重ねる |
| `.hero__media::after`（1200px以上） | `linear-gradient(to right, rgba(0,0,0,.74) 0%, rgba(0,0,0,.66) 45%, rgba(0,0,0,.28) 100%)` |
| 見出し | `#fff` |
| サブコピー | `rgba(255,255,255,.88)` |
| CTA下の注釈 | `rgba(255,255,255,.80)` |
| CTAボタン | 濃緑のまま。`border-color: rgba(255,255,255,.7)` と `box-shadow` で輪郭を確保 |

スマホでは**テキストが下寄せで全幅に広がる**ため、縦方向（下を濃く）を主にしています。
横方向だけにすると本文の右端が明るい窓側に重なり、実測で 2.51 まで落ちました。
文字色の変更は `.hero__title` / `.hero__lead` / `.hero__actions .btn-note` に限定しており、
**02以降のセクションの黒文字（`--ink`）は変更していません**（計測でも `rgb(27,37,35)` のままであることを確認）。

#### コントラストの実測

ヘッドレスChromeで**文字だけを透明にした状態のスクリーンショットを撮り、文字の行ボックスに
当たるピクセルだけを採取**して算出しています。白抜き文字は半透明なので、
各ピクセルごとに**実効の文字色 = a×255 + (1−a)×背景**を求めて比を取っています。

| 検証幅 | 見出し（#fff） | サブコピー（.88） | 注釈（.80） |
|---|---|---|---|
| 375×667 / 375×812 | 5.09 | 6.61 | 10.3 |
| 390×844 | 6.02 | 7.54 | 10.3 |
| 412×823 | 6.44 | 7.65 | 10.3 |
| 600px | 13.7 | 11.3 | 10.2 |
| 800px | 11.6 | 10.9 | 9.86 |
| 1024px | 10.2 | 11.2 | 9.63 |
| 1200px | 11.4 | 6.22 | 7.85 |
| 1280px（動画再生中も同値） | 11.0 | 6.22 | 8.06 |
| 1920px | 9.88 | 6.17 | 6.65 |

いずれも**最小値**（もっとも条件の悪い1ピクセル）です。全体の最小は 375px の見出し 5.09、
本文系はすべて 6.17 以上で、AA（4.5:1）を満たしています。

**Lighthouse / axe は画像の上に乗る文字のコントラストを判定できない**ため、この計測を別途行っています。
写真を差し替えたときは同じ手順で再確認してください。
...
| エラー分岐＊ | `network` / `empty` / `conflict` それぞれ別の表示とイベント（`conflict` は送信を伴う） |
...
**＊ 印の項目は実際の POST 送信を伴うため、Cloudflare Turnstile の実チャレンジが必要です。**
最大8秒（`TURNSTILE_WAIT_MS`）待ってもトークンが得られない場合、そのテストは実際の送信を行わず
`turnstileTokenObtained: false` を結果に含めて以降の手順をスキップします（アプリのバグではなく、
Cloudflare がヘッドレスブラウザを区別する仕様によるもの。詳細は「既知の制約」を参照）。
これらの項目を確実に検証したい場合は、実ブラウザで手動確認してください。
...
- **Cloudflare Turnstile にバイパスは設けていません。** ウィジェットの許可ホストに
  `wada3333.github.io` と `localhost` の両方を登録しているため、ローカル（`http://localhost:PORT/`）
  でも実チャレンジが解決してトークンが取得できます。**`127.0.0.1` は別ホスト扱いのため登録しておらず、
  `http://127.0.0.1:PORT/` で開くとチャレンジがエラーになります**（コンソールに
  `[Cloudflare Turnstile] Error: ...` が出ます）。`tools/serve.mjs` は `127.0.0.1` にバインドしていますが、
  ブラウザで開く際は `http://localhost:8123/` を使ってください
- **ヘッドレスブラウザでは実チャレンジが完了しないことがあります。** Cloudflare 側の自動化検知により、
  `localhost` からでも Turnstile が反応を返さない（＝トークンが永遠に来ない）ケースを確認しています。
  `tools/e2e-check.mjs` の予約送信系テスト（正常系・キーボードのみの完了・conflict分岐）は、
  一定時間待ってもトークンが得られない場合は実際に送信せず `turnstileTokenObtained: false` として
  結果に明示し、以降の手順をスキップします（アプリのバグではなく、Turnstile が自動化ブラウザを
  区別する仕様によるものです）。この3項目は実ブラウザでの手動確認、または Cloudflare が提供する
  テスト用サイトキーに差し替えての確認を想定しています
